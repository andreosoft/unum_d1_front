...
const actions = {
  signUp({ commit, dispatch, rootState }, data) {
    return dispatch('confirmLogin', data.user.login).then((status) => {
      if (!status) {
        return axios.post(api.doctorSignUp, data).then((res) => {
          dispatch('login', {
            ...
            commit('SET_DOCTOR_MEDICAL_UNIVERSITY', data.education.medical_university);
            commit('SET_DOCTOR_MEDICAL_SPECIALTY', data.education.medical_specialty);
            commit('SET_DOCTOR_DOCTOR_SPECIALTY', data.education.doctor_specialty);
            commit('SET_DOCTOR_YEARS_OF_EDUCATION', data.education.years_of_education);
      ...
      dispatch(
        ...
        {
          type: 'error',
          text: rootState.lang.common['Email taken'] ? rootState.lang.common['Email taken'] : 'Email taken',
        },
        { root: true }
      );
    });
    ...
    return axios.post(api.doctorLogin, { login, password }).then(async (res) => {
      if (res.data.status === 'error' || res.data.data.profile.role !== 100) {
        dispatch(
          ...
          {
            type: 'error',
            text: rootState.lang.common['Credentials error'] ? rootState.lang.common['Credentials error'] : 'Credentials error',
          },
          { root: true }
        );
        return;
  ...
  confirmLogin({ commit, dispatch }, login) {
    return axios.get(api.confirmLogin, { params: { login } }).then(async (res) => {
      if (!res.data.data) {
  ...
  fetchUserProfile({ state, commit, dispatch, rootState }) {
    return axios.get(api.getUserProfile).then((res) => {
      try {
        ...
        const errorText = rootState.lang.common['Profile not found'] ? rootState.lang.common['Profile not found'] : 'Profile not found';
  ...
  logout({ commit }, lang = 'en') {
    ...
    commit('SET_DOCTOR_PROFILE', null);
